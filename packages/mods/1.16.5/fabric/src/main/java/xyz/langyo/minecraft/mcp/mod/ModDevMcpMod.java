    ...
    private static McpRenderer wrapRenderer(MatrixStack ms, MinecraftClient mc) {
        return new McpRenderer() {
            ...
            @Override public int drawString(Object font, String text, int x, int y, int color, boolean shadow) {
                if (shadow) {
                    return mc.textRenderer.drawWithShadow(ms, text, x, y, color);
                } else {
                    return mc.textRenderer.draw(ms, text, x, y, color);
                }
            }
            ...
            try {
                ...
                if (mc.currentScreen == null) {
                    ...
                    if (ReflectionHelper.isMcpControlMode() && !ReflectionHelper.isScreenshotInProgress()) {
                        ...
                        McpOverlayLogic.renderResumeButton(wrapRenderer(matrixStack, mc), mc.textRenderer, new TranslatableText("mcpmod.control.resume").getString(), w, h, (int) mx, (int) my);
                    }
                    ...
                    double mx = mc.mouse.getX() * w / mc.getWindow().getWidth();
                    double my = mc.mouse.getY() * h / mc.getWindow().getHeight();
                    McpOverlayLogic.renderTransferButton(wrapRenderer(matrixStack, mc), mc.textRenderer, new TranslatableText("mcpmod.control.pause_button").getString(), w, h, (int) mx, (int) my);
                }
                ...
                if (screen instanceof GameMenuScreen) {
                    McpScreenHelper.patchPauseScreen(screen, new McpScreenHelper.ButtonFactory() {
                        @Override public Object createButton(String translationKey, Runnable onClick, int x, int y, int w, int h) {
                            return new ButtonWidget(x, y, w, h, new TranslatableText(translationKey), btn -> onClick.run());
                        }
                    });
                }
            } catch (Exception ignored) {}

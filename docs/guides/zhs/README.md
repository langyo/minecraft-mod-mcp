...
> 想让你的 AI 测试你的模组 GUI？跑一遍冒烟测试？验证新方块的交互逻辑？Minecraft Mod MCP 让这一切成为可能。纯粹的 Minecraft 玩法自动化，其他专用工具做得更好。
...
模组启动时会在 `http://localhost:9876` 上运行 HTTP 服务器，并提供一个实时调试面板——上方截图展示的就是这个页面。在浏览器中打开它，即可在编码的同时查看 MCP 日志、连接状态等实时诊断信息。

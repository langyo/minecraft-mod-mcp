...
> AI가 모드 GUI를 테스트하게 하고 싶으신가요? 스모크 테스트를 실행하고 싶으신가요? 새로운 블록의 상호작용을 검증하고 싶으신가요? Minecraft Mod MCP가 가능하게 합니다. 순수 게임플레이 자동화는 다른 전용 도구에 맡기세요.
...
게임 시작 시 `http://localhost:9876`에서 HTTP 서버가 실행되며, 실시간 디버그 대시보드가 제공됩니다——위의 스크린샷이 바로 그 화면입니다. 브라우저에서 열면 코딩하면서 MCP 로그, 연결 상태 등 실시간 진단 정보를 확인할 수 있습니다.

...
> AIにModのGUIをテストさせたい？スモークテストを実行したい？新しいブロックの動作を検証したい？Minecraft Mod MCPなら可能です。純粋なゲームプレイ自動化には、他の専用ツールが適しています。
...
ゲーム起動時に、`http://localhost:9876` で HTTP サーバーが実行され、リアルタイムのデバッグダッシュボードが提供されます——上記のスクリーンショットがその画面です。ブラウザで開くと、コーディングしながら MCP ログや接続状態などのリアルタイム診断情報を確認できます。

...
## Minecraft Mod MCP HTTP 端點

Minecraft Mod MCP 伺服器提供以下 HTTP 端點（預設連接埠：**9876**）：
...
對於本身不支援 MCP 協議的工具，您可以透過 HTTP REST API 直接與 Minecraft Mod MCP 伺服器互動：
...
你可以將 Minecraft Mod MCP 與**具備視覺能力的 MCP 伺服器**搭配使用，讓 AI 代理能夠*看見並理解*遊戲中發生的事情 — 讀取 UI 文字、診斷錯誤、分析版面配置等。

### 運作原理

1. Minecraft Mod MCP 擷取螢幕截圖，並透過 `screenshot_to_file` 儲存為本地檔案

# Minecraft 1.12.2 Forge 注入原理

[English](../en/1.12.2+forge.md) | [中文](1.12.2+forge.md)

## 概述

Minecraft 1.12.2 Forge 的 MCP 模组使用 **Forge 事件总线** 系统进行注入。这属于旧版 Forge 时代（ForgeGradle 2.x），早于 `mods.toml` 的引入，使用 `@Mod.EventHandler` 注解代替。注入完全依赖 `@SubscribeEvent` 监听器和一个自定义的 `MouseHelper` 替换来实现输入控制。**没有使用 Mixin**，也**没有 coremod**——一切完全由事件驱动。
...
这是旧版 Forge 注入**最显著的特征**。由于 Minecraft 1.12.2 使用 **LWJGL2**（而非 GLFW），鼠标通过 `net.minecraft.util.MouseHelper` 管理，而不是 GLFW 回调。
...
- **Forge 1.12.2** 使用 ForgeGradle 2.x 和 LWJGL2

# Minecraft 1.15.2 Fabric 注入原理

[English](../en/1.15.2+fabric.md) | [中文](1.15.2+fabric.md)

## 概述

Minecraft 1.15.2 Fabric 的 MCP 模组使用 **SpongePowered Mixin** 字节码注入来挂钩原版 Minecraft 客户端的渲染循环和输入处理。Mixin 框架通过 Fabric Loader 在编译期进行字节码织入，通过带注解的混入类将方法调用重定向。
...
- **Java 8**（但使用 Java 16 编译）：1.15.2 版本使用 `JAVA_8` 兼容级别，但以 Java 16 编译

# Minecraft 1.16.5 Forge 注入原理

[English](../en/1.16.5+forge.md) | [中文](1.16.5+forge.md)

## 概述

适用于 Minecraft 1.16.5 Forge 的 MCP 模组使用 **Forge 事件总线**系统与 **GLFW 后端**。这是现代 Forge 时代（ForgeGradle 3+），引入了 `mods.toml`、通过 `FMLJavaModLoadingContext` 进行基于 Lambda 表达式的事件注册，以及 GLFW 鼠标管理。从 LWJGL2 到 GLFW 的过渡（始于 1.13）从根本上改变了鼠标输入的处理方式——用 GLFW 回调拦截取代了 `MouseHelper` 字段替换模式。
...
- **1.16.5**：Forge 36.2.34，Java 8。最稳定且使用最广泛的现代 Forge 版本。完整支持 `ensureMouseInterceptor()`。最广泛的模组生态兼容性。

# Minecraft 1.18.2 Forge 注入原理

[English](../en/1.18.2+forge.md) | [中文](1.18.2+forge.md)

## 概述

适用于 Minecraft 1.18.2 Forge 的 MCP 模组使用 **Forge 事件总线**系统与 **GLFW 回调拦截**实现完整的鼠标控制。这是成熟的现代 Forge 时代（ForgeGradle 6.x），包含 `mods.toml`、基于 Lambda 表达式的事件注册，以及全面的 GLFW 级别鼠标管理。由于完备的鼠标输入拦截系统，版本 1.18.2 代表了最复杂的 Forge 注入实现。
...
- **1.18.2**：Forge 40.2.0，Java 17。与 1.20.6 架构相同，但运行在 Java 17 上。

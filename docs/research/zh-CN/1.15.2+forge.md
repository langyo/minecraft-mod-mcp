# Minecraft 1.15.2 Forge 注入原理

[English](../en/1.15.2+forge.md) | [中文](1.15.2+forge.md)

## 概述

适用于 Minecraft 1.15.2 Forge 的 MCP 模组使用 **Forge 事件总线**系统与 **GLFW 后端**。这是现代 Forge 时代（ForgeGradle 3+），引入了 `mods.toml`、通过 `FMLJavaModLoadingContext` 进行基于 Lambda 表达式的事件注册，以及 GLFW 鼠标管理。从 LWJGL2 到 GLFW 的过渡（始于 1.13）从根本上改变了鼠标输入的处理方式——用 GLFW 回调拦截取代了 `MouseHelper` 字段替换模式。
...
- **1.15.2**：Forge 31.2.60，Java 8。需要 ForgeGradle 6.0+。具有成熟的 `ensureMouseInterceptor()` 实现。

# Minecraft 1.13.2 Forge 注入原理

[English](../en/1.13.2+forge.md) | [中文](1.13.2+forge.md)

## 概述

适用于 Minecraft 1.13.2 Forge 的 MCP 模组使用 **Forge 事件总线**系统与 **GLFW 后端**。这是现代 Forge 时代（ForgeGradle 3+），引入了 `mods.toml`、通过 `FMLJavaModLoadingContext` 进行基于 Lambda 表达式的事件注册，以及 GLFW 鼠标管理。从 LWJGL2 到 GLFW 的过渡（始于 1.13）从根本上改变了鼠标输入的处理方式——用 GLFW 回调拦截取代了 `MouseHelper` 字段替换模式。
...
**注意**：在 1.13.2 中，GLFW 鼠标拦截器**尚未实现**。此过渡版本仅依赖事件取消（例如 `ScreenEvent.Opening.setCanceled(true)`）来阻止输入。完整的 GLFW 回调拦截器（`ensureMouseInterceptor()`）从 1.14.4 开始引入。
...
- **1.13.2**：Forge 25.0.223，Java 8。首个基于 GLFW 的版本（过渡性）。鼠标处理更简单——依赖事件取消，而非 GLFW 回调拦截。没有 `ensureMouseInterceptor()`。
...
- **1.17.1**：Forge 37.1.1，Java 16。基于深度的渲染变更开始。
